...
public class TellerManagementReadPlatformServiceImpl implements TellerManagementReadPlatformService {
    ...
    public Collection<TellerData> retrieveAllTellers(final boolean includeAllTellers) {
        ...
        //Yves FOPA - 23 Oct 2025

        //Changing the logic here so it works as follow
        //We need to list all tellers (means all tellers for all offices) only if the user has TellerAdministrator role
        //and for others limit tellers list to user banch/office only

        final String hierarchy = this.context.officeHierarchy();//currentUser.getOffice().getHierarchy(); 
        String hierarchySearchString = null;

        final Set<Role> userRoles = currentUser.getRoles();
        List<Role> rolesList = new ArrayList<Role>();
        rolesList.addAll(userRoles);
        //if(roles.isEmpty()) - we don't check this as user must have a role
        final String role_name = rolesList.getFirst().getName(); //TellerAdministrator should be the only user role, that's why we pick first element here
        if(role_name.equals("TellerAdministrator")){
            hierarchySearchString = "." + "%";
        } else{
            hierarchySearchString = hierarchy + "%";
        }
        //end of Yves FOPA added code

         //commented out this original code bloc - Yves FOPA 23 Oct 2025
